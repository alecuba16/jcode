...
enum Disposition {
    ...
    /// that says what a client cannot build without it. The ledger currently
    /// has zero Gap entries; the variant stays so adding one is a one-line
    /// change and the exhaustive match below keeps its reporting arm.
    #[allow(dead_code)]
    Gap(&'static str),
}
