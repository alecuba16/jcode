...
const DEFAULT_REFRESH_TTL_SECS: u64 = 30;
...
impl FileMentionCache {
        ...
        for (wi, manager) in self.managers.iter().enumerate() {
            let Some(mut results) = manager.with_index(|index| {
                if !index.built {
                    ...
                    Some(search_in_index(query, index, &self.frecency))
                }
            }) else {
            ...
            if !q.is_empty() && results.len() < MAX_RESULTS && manager.ensure_ignored_dir_scanned(q)
            {
                results = manager.with_index(|index| search_in_index(query, index, &self.frecency));
            }
    ...
    pub fn check_refresh(&mut self, cwd: &Path) {
        ...
        for manager in &self.managers {
            ...
            let refresh_ttl_secs = crate::config::Config::load().file_mention_refresh_ttl_secs;
...
mod tests {
    ...
    fn refresh_ttl_small_count_uses_base_interval() {
        let base = Duration::from_secs(DEFAULT_REFRESH_TTL_SECS);
        assert_eq!(refresh_ttl(0, DEFAULT_REFRESH_TTL_SECS), base);
        assert_eq!(refresh_ttl(100, DEFAULT_REFRESH_TTL_SECS), base);
        assert_eq!(
            refresh_ttl(REFRESH_TTL_LARGE_THRESHOLD - 1, DEFAULT_REFRESH_TTL_SECS),
            base
        );
    }
    ...
    fn refresh_ttl_large_count_uses_large_interval() {
        let large = Duration::from_secs(DEFAULT_REFRESH_TTL_SECS * 4);
        assert_eq!(
            refresh_ttl(REFRESH_TTL_LARGE_THRESHOLD, DEFAULT_REFRESH_TTL_SECS),
            large
        );
        assert_eq!(refresh_ttl(10_000, DEFAULT_REFRESH_TTL_SECS), large);
    }

    #[test]
    fn refresh_ttl_changes_at_threshold() {
        assert_ne!(
            refresh_ttl(100, DEFAULT_REFRESH_TTL_SECS),
            refresh_ttl(10_000, DEFAULT_REFRESH_TTL_SECS)
        );
        assert_eq!(
            refresh_ttl(0, DEFAULT_REFRESH_TTL_SECS),
            refresh_ttl(100, DEFAULT_REFRESH_TTL_SECS)
        );
    }
    ...
    fn refresh_ttl_zero_config_uses_default() {
        assert_eq!(
            refresh_ttl(0, 0),
            Duration::from_secs(DEFAULT_REFRESH_TTL_SECS)
        );
    }
