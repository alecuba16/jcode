...
fn test_improve_mode_persists_in_session_file() {
    with_temp_jcode_home(|| {
        ...
        // Bare sessions without a visible message or title are deliberately
        // not persisted (lazy-save guard), so give this one an explicit title.
        session.title = Some("improve-mode persistence check".to_string());
