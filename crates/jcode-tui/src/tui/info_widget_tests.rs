use super::{
    ...
    render_memory_compact, render_memory_widget, render_recovered_memories_widget, render_sections,
...
#[test]
fn overview_sections_show_effort_bracket_for_reasoning_model() {
    // The bordered Overview renders the model section via render_sections ->
    // render_model_info: provider line, then `(hi) Model` on the next line.
    ...
    let lines = render_sections(&data, Rect::new(0, 0, 40, 10), None);
    ...
    assert!(
        ...
        "reasoning effort bracket missing from Overview model line: {model_line}"
    );
}

#[test]
fn overview_sections_omit_effort_bracket_for_plain_model() {
    ...
    assert!(
        ...
        "model line missing from Overview: {joined}"
    );
}
...
fn render_kv_cache_summary_line_renders_session_ratio_only_without_last_stats() {
    // Subset accounting (cached tokens are inside the reported input), with
    // an explicit prompt size: hit_ratio uses prompt_tokens (never inferred
    // from aggregate counters) as the denominator.
    let cache = CacheHitInfo {
        prompt_tokens: Some(1_000),
        reported_input_tokens: 1_000,
        read_tokens: 800,
...
/// Improvement C, height half: the reserved Overview height must not grow
/// with the streaming t/s value, because `render_sections` has no t/s line
/// (the live rate renders on the status line, the rolling average in the
/// cost line suffix). A phantom reserved row leaves a blank gap above the
/// border once the panel is full.
#[test]
fn compact_overview_height_ignores_streaming_tps() {
    ...
    let streaming = InfoWidgetData {
        tokens_per_second: Some(42.0),
        ..base.clone()
    };

    let base_h = super::compute_page_layout(&base, 38, 30).max_page_height;
    let streaming_h = super::compute_page_layout(&streaming, 38, 30).max_page_height;
    assert_eq!(
        base_h, streaming_h,
        "streaming t/s must not reserve an extra Overview row: {base_h} vs {streaming_h}"
    );

    // And symmetrically: rendered line count must match reserved height, so
    // the t/s sample cannot clip a later section either.
    let inner = Rect::new(0, 0, 38, 30);
    let base_lines = super::render_page(super::InfoPageKind::CompactOnly, &base, inner);
    let streaming_lines = super::render_page(super::InfoPageKind::CompactOnly, &streaming, inner);
    assert_eq!(base_lines.len(), streaming_lines.len());
}

/// Improvement C, render half: when the user hides t/s, the cost line must
/// not grow or shrink — it must be exactly the same width with and without
/// a rolling average present. With t/s shown (default), a present average
/// must render the suffix.
#[test]
fn cost_line_tps_suffix_respects_hide_flag() {
    ...
    let text = line_text(&render_cost_tokens_line(&data, rect));
    assert!(text.contains("⌀38.2 t/s"), "{text}");

    let hidden = InfoWidgetData {
        hide_tps: true,
        ..data.clone()
    };
    let hidden_text = line_text(&render_cost_tokens_line(&hidden, rect));
    assert!(
        !hidden_text.contains("t/s"),
        "cost line must drop the t/s suffix when hidden, got: {hidden_text}"
    );
}
