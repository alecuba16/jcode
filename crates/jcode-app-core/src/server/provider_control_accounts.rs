// Auth-change and account-switch request handling for the provider
// control plane. Child module of provider_control (see the #[path]
// declaration there) so the parent stays under the code-size budget.
use super::*;

#[allow(clippy::too_many_arguments)]
pub(in crate::server) async fn handle_notify_auth_changed(
    id: u64,
...
pub(in crate::server) async fn handle_switch_anthropic_account(
    id: u64,
    label: String,
    agent: &Arc<Mutex<Agent>>,
    client_event_tx: &mpsc::UnboundedSender<ServerEvent>,
...
pub(in crate::server) async fn handle_switch_openai_account(
    id: u64,
    label: String,
    agent: &Arc<Mutex<Agent>>,
    client_event_tx: &mpsc::UnboundedSender<ServerEvent>,
...
pub(in crate::server) async fn handle_invalidate_openai_usage(
    id: u64,
...
pub(in crate::server) async fn handle_invalidate_anthropic_usage(
    id: u64,
    account_label: Option<String>,
    client_event_tx: &mpsc::UnboundedSender<ServerEvent>,
) {
